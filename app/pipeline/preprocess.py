...
def crop_head_shoulders(img: Image.Image, face: FaceInfo, out_size: int) -> Image.Image:
    ...
    Вертикальная позиция задаётся ЧЕРЕЗ ДОЛЮ от стороны кропа (подбородок на ~58% высоты вниз
    от верха — так и получается 0.75fh запаса над головой + 1fh до подбородка при исходной
    стороне 3fh), а не фиксированным отступом от макушки. Если бы отступ был фиксированным,
    а сторону при этом обрезали под размер фото (см. выше), нижняя граница могла оказаться
    ВЫШЕ подбородка — то есть лицо срезалось бы снизу. Расчёт через долю гарантирует, что
    подбородок остаётся внутри кадра при любом урезании стороны.
    ...
    CHIN_RATIO = 1.75 / 3.0  # доля стороны от верха кропа до подбородка (замысел исходной раскладки)

    side = int(round(min(3.0 * fh, w, h)))
    cx = face.center[0]
    top = int(round(face.chin_y - CHIN_RATIO * side))
